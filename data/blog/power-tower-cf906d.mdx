...
## Lemma 1 proof
Before proving this crazy thing, the following lemma is required. Beware that the proof won't isn't clean, as it seems like it is accidentally true.

Firstly, note that $\forall a,b: a @ b \le b$. Secondly, let's prove this lemma first:  
$$
...
* ## Case 2: $a^b < 4m, a^{b @ \phi(m)} \ge 4m$
  Since $b @ \phi(m) \le \Rightarrow a^{b @ \phi(m)} \le a^b$. So this case never occurs.
